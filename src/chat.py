...
class PersonalRAGChat:
    def __init__(self, retriever: Optional[DocumentRetriever] = None):
        ...
        self.config = Config()
        logger.info(
            f"📋 Configuration loaded - LLM Provider: {self.config.LLM_PROVIDER}"
        )
    ...
    def ask_question(self, question: str, include_sources: bool = True) -> Dict:
        ...
        try:
            ...
            if hasattr(self.memory, "chat_memory") and hasattr(
                ...
                chat_history = "\n".join(
                    [
                        f"Human: {msg.content}" if hasattr(msg, "content") else str(msg)
...
def main():
    ...
    try:
        ...
        print(
            "Type 'quit' to exit, 'clear' to clear history, 'history' to see conversation history."
        )
            ...
            try:
                ...
                if question.lower() in ["quit", "exit", "q"]:
                    ...
                    if provider == "gemini":
                        ...
                        print(
                            f"   🔑 API Key: {'✅ Set' if chat.config.GEMINI_API_KEY else '❌ Missing'}"
                        )
